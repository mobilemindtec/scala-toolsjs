...
object DatePicker:
  ...
  extension (jq: JQuery)
    ...
    def dtpickerMaybe(format: String = "dd/mm/yyyy", startDate: Option[LocalDate] = None)(
      changeDate: Option[LocalDate] => Unit
    ): JQuery =
      ...
      jq.on(
        ...
        { (rawEvt: JQueryEventObject) =>
          ...
          val date = 
            if changeDate.nonEmpty
            then
              if rawEvt.asInstanceOf[DatepickerEventObject].date != js.undefined 
              then
                val dt = rawEvt.asInstanceOf[DatepickerEventObject].date.toLocalDate
                Some(dt)
              else None
